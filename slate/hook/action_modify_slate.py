'''Modify existing slate action for Ftrack
Alican Sesli - Lux VFX - 03.02.2020 - 04.02.2020
    v01 - proting in from slate assets v28
    v02 - works
    v03 - slates need to go to a folder named slates on ftrack
'''
...
class DuplicateSlate(object):
    '''Custom action.'''

    label = 'Slate'
    identifier = 'modify.a.slate'
    description = 'This will modify an existing slate.'
    icon = 'https://d30y9cdsu7xlg0.cloudfront.net/png/1465042-200.png'
    variant = 'Modify'

    def __init__(self, session):
        '''Initialise action.'''
        super(DuplicateSlate, self).__init__()
    ...
    def launch(self, event):
        ...
        slate = session.query('select name, custom_attributes from Slate where id is {}'.format( entity['entityId'] )).first()

        slate_options = slate['custom_attributes']
        ...
        all_projects = session.query('select name, full_name, id from Project').all()

        current_project = session.query('select project.full_name from TypedContext where id is {}'.format( entity['entityId'] )).first()

        current_project = current_project['project']['full_name']
        ...
        for p in all_projects:
            project_names.append( p['full_name'] )

        project_names = sorted(list(set(project_names)))


        for p in project_names:
            project_list_dct = {'label':p, 'value':p}
            project_list.append(project_list_dct)
        ...
        slate_settings = [{
                    ...
                    'label': '<b><font color="green">Project</font></b>',
                    'type': 'enumerator',
                    ...
                    'type': 'text',
                    'value': slate['name'],#Type a new name or choose from existing...',
                    'data' : '',
                    'name': 'slate_name'
                },{
                    'label': '<b><font color="green">Slate Type</font></b>',
                    'type': 'enumerator',
                    'value': slate_options['slate_type'],#'offline',#slate_types[0]['label'],
                    'data' : slate_types,
                    'name': 'slate_type'
                },{
                    'label': ' + <i>Slate Codec</i>',
                    'type': 'enumerator',
                    'value': slate_options['slate_codec'],#'Avid DNxHD Codec',#proj['custom_attributes']['slate_codec_offline'],
                    'data' : slate_codecs,
                    'name': 'slate_codec'
                },{
                    'label': ' + <i>Slate Format</i>',
                    'type': 'enumerator',
                    'value': slate_options['slate_format'],#'HD_1080  1920 x 1080',#proj['custom_attributes']['slate_format_offline'],
                    'data' : slate_formats,
                    'name': 'slate_format'
                },{
                    'label': ' + <i>Slate Output</i>',
                    'type': 'text',
                    'value': slate_options['slate_output'],#'{defaultoutput}/{slatetype}/{codec}/{assetname}.mov',#proj['custom_attributes']['slate_output_offline'],
                    'name': 'slate_output'
                },{
                    'label': ' + <i>Slate Template</i>',
                    'type': 'enumerator',
                    'name': 'slate_template',
                    'data': slate_options['slate_template'],#template_items,
                    'value': slate_options['slate_template']#'Generic_Slate_MOV.nk',#proj['custom_attributes']['offline_slate_template']
                },{
                    'label': '<font color="yellow"> + <i>Colorspace In</i><font>',
                    'type': 'enumerator',
                    'name': 'slate_colorspace_in',
                    'data': slate_colorspaces,
                    'value': slate_options['slate_colorspace_in']#'sRGB',#proj['custom_attributes']['slate_colorspace_in_offline']
                },{
                    'label': '<font color="yellow"> + <i>Colorspace Out</i><font>',
                    'type': 'enumerator',
                    'name': 'slate_colorspace_out',
                    'data': slate_colorspaces,
                    'value': slate_options['slate_colorspace_out']#'sRGB'#proj['custom_attributes']['slate_colorspace_out_offline']
                },{
                    'label': '<font color="orange"> + <i>Data Type</i><font>',
                    'type': 'enumerator',
                    'name': 'slate_data_type',
                    'data': data_type_settings,
                    'value': slate_options['slate_data_type']#'default',#proj['custom_attributes']['slate_data_type_offline']
                },{
                    'label': '<font color="pink"> + <i>Slate Frame</i></font>',
                    'type': 'boolean',
                    'value': slate_options['slate_slateframe'],#True, #proj['custom_attributes']['offline_slate_frame'],
                    'name': 'slate_slateframe'
                },{
                    'label': '<font color="pink"> + <i>Overlays</i></font>',
                    'type': 'boolean',
                    'value': slate_options['slate_overlays'],#True, #proj['custom_attributes']['offline_overlays'],
                    'name': 'slate_overlays'
                },{
                    'value': '<hr>',
                    'type': 'label'
...

                {
                    ...
                    'type': 'boolean',
                    'value': slate_options['slate_rename'],#True,#proj['custom_attributes']['slate_rename_for_client'],
                    'name': 'rename_for_client'
                },{
                    'label': ' + <i>Rename with Tags</i>',
                    'type': 'text',
                    'value': slate_options['slate_assetname_rename'],#'{project}{episode}_{sequence}_{shot}_v{version}',#proj['custom_attributes']['slate_rename_with_tags'],
                    'name': 'rename_text'
                },{
                    'value': '<hr>',
                    'type': 'label'
                    ...
                    'type': 'text',
                    'value': slate_options['slate_overlays_description_text'],#'Client Review',#proj['custom_attributes']['slate_description_text'],
                    'name': 'description_text'
                },{
                    'label': 'Custom Text',
                    'type': 'text',
                    'value': slate_options['slate_overlays_custom_text'],#'',#proj['custom_attributes']['slate_custom_text'],
                    'name': 'custom_text'
                },{
                    'value': '<hr>',
                    'type': 'label'
                    ...
                    'type': 'enumerator',
                    'value': slate_options['slate_slateframe_project_title'],#'{projectname}',#proj['custom_attributes']['slate_frame_project_title'],
                    'data' : slate_txt_positions,
                    'name': 'slate_frame_project_title'
                },{
                    'label': '<font color="pink">Slate Info</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_slateframe_description'],#'{description} : {date}',#proj['custom_attributes']['slate_frame_description'],
                    'data' : slate_txt_positions,
                    'name': 'slate_frame_description'
                },{
                    'label': '<font color="pink">Shot Title</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_slateframe_shot_title'],#'{project}{episode}_{sequence}_{shot}',#proj['custom_attributes']['slate_frame_shot_title'],
                    'data' : slate_txt_positions,
                    'name': 'slate_frame_shot_title'
                },{
                    'label': '<font color="pink">Frame Range Info</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_slateframe_range_info'],#'{duration} frames',#proj['custom_attributes']['slate_frame_range_info'],
                    'data' : slate_txt_positions,
                    'name': 'slate_frame_range_info'
                },{
                    'label': '<font color="pink">Timecode Info</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_slateframe_timecode_info'],#'{timecode}',#proj['custom_attributes']['slate_frame_timecode_info'],
                    'data' : slate_txt_positions,
                    'name': 'slate_frame_timecode_info'
                },{
                    'label': '<font color="pink">Version Info</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_slateframe_version_info'],#'{task} Version {version}',#proj['custom_attributes']['slate_frame_version_info'],
                    'data' : slate_txt_positions,
                    ...
                    'type': 'enumerator',
                    'value': slate_options['slate_overlays_top_left'],#'{logo}',#proj['custom_attributes']['slate_top_left'],
                    'data' : slate_txt_positions,
                    'name': 'slate_top_left'
                },{
                    'label': '<font color="pink">Top Right</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_overlays_top_right'],#'{date}',#proj['custom_attributes']['slate_top_right'],
                    'data' : slate_txt_positions,
                    'name': 'slate_top_right'
                },{
                    'label': '<font color="pink">Bottom Left</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_overlays_bottom_left'],#'{assetname}',#proj['custom_attributes']['slate_bottom_left'],
                    'data' : slate_txt_positions,
                    'name': 'slate_bottom_left'
                },{
                    'label': '<font color="pink">Bottom Right</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_overlays_bottom_right'],#'{framecounter}',#proj['custom_attributes']['slate_bottom_right'],
                    'data' : slate_txt_positions,
                    'name': 'slate_bottom_right'
                },{
                    'label': '<font color="pink">Top Center</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_overlays_top_center'],#'{none}',#proj['custom_attributes']['slate_top_center'],
                    'data' : slate_txt_positions,
                    'name': 'slate_top_center'
                },{
                    'label': '<font color="pink">Bottom Center</font>',
                    'type': 'enumerator',
                    'value': slate_options['slate_overlays_bottom_center'],#''{description}',#proj['custom_attributes']['slate_bottom_center'],
                    'data' : slate_txt_positions,
        ...
        if 'values' in data:
            ...
            slate = session.query('Slate where name is "{}" and project.id is {}'.format(slate_name, project_id)).all()

            project_name = values['to_project']
            proj = session.query('select id, name, full_name from Project where full_name is {}'.format( project_name )).first()


            slate_folder = session.query('Folder where project.id is {} and name is "Slates"'.format(proj['id'])).all()
            if not slate_folder:
            ...
            retSuccess = {
                'success': True,
                'message': '{} created under {}!'.format(slate_name, project_name)
            }
