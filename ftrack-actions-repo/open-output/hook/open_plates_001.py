'''
Open plates Ftrack Hook
Alican Sesli - LUX VFX - 01.29.2020 - 01.29.2020
Opens the Plates folder of a selected item in Windows Explorer
    v1: first version
'''
...
class OpenOutput(object):
    ...
    def launch(self, event):
        ...
        for entity in selection:
            ...
            task = self.session.query('select custom_attributes, object_type.name, parent.id, parent.custom_attributes from TypedContext where id is "{0}"'.format(entityId)).first()
            ...
            #print task['object_type']['name']

            if task['object_type']['name'] == 'Task':

                shot_path = task['parent']['custom_attributes']['out_path']
